...
<body>
	...
	<!-- DIV PER INSERIMENTO O MODIFICA DELLA MATRICE O DEL VETTORE DEI PESI -->
...
<!-- 				<input type="text" placeholder="false" id="false2" maxLength=5 size=6> -->
	...
	<!-- DIV PER INSERIMENTO OGGETTO DELL'ISTANZA -->
...
// 			changeSelectIFPart1();
		...
		function new_instance(specific_object)
		{
			id_currentpage = 5;
			...
			// Per ogni nodo si inserisce un nodo Italian Flag modify
			node.each(function(d,i){
				if(getId(d.position) != 0)
					addNodeIFEmpty(getId(d.position), d.description, specific_object, null);
				else
			...
			{
				d3.select("svg").remove();
				showAlert("It's not possible create a new instance!\nThe linked model have a criteria with one child");
				
				disableSubMenu();
				
				clientRest.deleteModelInstance();
			}
		}
