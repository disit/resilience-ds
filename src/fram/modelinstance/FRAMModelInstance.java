...
public class FRAMModelInstance extends FRAMModel{
	/*
	 * Ale: Class for a FRAM instance.
	 */
	private int modelInstanceId;
	
	@XmlElement(name="specific_objective")
...
//	private Timestamp timestamp_end_exec;
	...
	private FRAMModel FatherModel; // TODO Check if is possible to delete this attribute
	
	private int instanceUserId;
	
	@XmlElementWrapper(name="FunctionInstances")
	@XmlElement(name="FunctionInstance")
	private ArrayList<FunctionInstance> FunctionsInstances; 
	
	public FRAMModelInstance()
	{
		
	}

	// Costruttore per creazione lista ModelInstance da scrivere nello stream
	public FRAMModelInstance(int modelInstanceId, String objective, int modelId, int userInstanceId)
	{
		super();
		super.setId(modelId);
		setMIId(modelInstanceId);
		setDescriptionInstance(objective);
		setUserInstanceId(userInstanceId);
	}
	// Costruttore per creazione lista ModelInstance da scrivere nello stream
	public FRAMModelInstance(int modelInstanceId, String objective, int modelId, int userInstanceId, String date_create, String date_modify)
	{
		super();
		super.setId(modelId);
		setMIId(modelInstanceId);
		setDescriptionInstance(objective);
		setUserInstanceId(userInstanceId);
		setDateCreateInstance(date_create);
		setDateLastModifyInstance(date_modify);
	}

	// Ale: Constructor that creates a FRAMModelInstance starting from the linked Model. 
	public FRAMModelInstance(int modelInstanceId, String objective, int userInstanceId, FRAMModel model)
	{
//		super(model.getId(), model.getObjective(), model.getSize(), model.getModelUserId());
		//XXX Doesn't copy the function XML in the file for prevent problem with XML.
		super(model);
		setMIId(modelInstanceId);
		setDescriptionInstance(objective);
		setUserInstanceId(userInstanceId);
		FunctionsInstances=new ArrayList<FunctionInstance>();
		if(model.getFunctions().size() > 0) initializeFunctionInstances(model.getFunctions());
	}
	
	
	public FRAMModelInstance(int modelInstanceId, String description, int UserId) {
		super();
		this.modelInstanceId = modelInstanceId;
		this.descriptionInstance= description;
		this.FatherModel=new FRAMModel();
		this.timestamp_create_instance = timestamp_create_instance;
		this.timestamp_last_modify_instance = timestamp_last_modify_instance;
		this.date_create_instance = date_create_instance;
		this.date_last_modify_instance = date_last_modify_instance;
		this.instanceUserId = UserId;
		this.FunctionsInstances = new ArrayList<FunctionInstance>();
	}
	/**
	 * Initialize the function instances with the function of the model with default values.
	 * @param Functions
	 */
	public void initializeFunctionInstances( ArrayList<Function> Functions){
		Functions.toString();
		
		for(int i=0; i < Functions.size(); i++){
			
			FunctionInstance fi = new FunctionInstance(Functions.get(i), -1, -1, 0);
			addFunctionInstance(fi);
		}
	}
	
	public FRAMModel getFatherModel() {
		return FatherModel;
	}

	public void setFatherModel(FRAMModel model) {
//		FatherModel = fatherModel;
		super.setId(model.getId());
		super.setObjective(model.getObjective());
		super.setDescriptionModel(model.getDescriptionModel());
		super.setPriority_type(model.getPriority_type());
		super.setSize(model.getSize());
		super.setTimestampCreateModel(model.getTimestampCreateModel());
		super.setTimestampLastModifyModel(model.getTimestampLastModifyModel());
		super.setDateCreateModel(model.getDateCreateModel());
		super.setDateLastModifyModel(model.getDateLastModifyModel());
		super.setModelUserId(model.getModelUserId());
//		super.Aspects=model.getAspects();
		super.setAspectsC(new ArrayList<AspectForClient>());
		super.setAspectsC(model.getAspectsC());
	}
	...
	public String getDescriptionInstance() {
		return descriptionInstance;
	}

	public void setDescriptionInstance(String description) {
		this.descriptionInstance = description;
	}
	...
	public ArrayList<FunctionInstance> getFunctionsInstances() {
		return FunctionsInstances;
	}
	
	//Ale: Searches for a specific instance.
	public FunctionInstance getFunctionInstance(int idF) {
		for(int i=0;i<FunctionsInstances.size();i++){
			if( FunctionsInstances.get(i).getId()==idF ) return FunctionsInstances.get(i);
		}
		return null;
	}
	
	public void setFunctionsInstances(ArrayList<FunctionInstance> functionsInstances) {
		FunctionsInstances = functionsInstances;
	}
	
	public void addFunctionInstance(FunctionInstance FI){
		FunctionsInstances.add(FI);
	}
	
	public String printFunctionInstance(){
		String Content="";
		for(FunctionInstance F:FunctionsInstances){
			Content+=F.toString();
		}
		return Content;
	}
    ...
    public String toString() {
        ...
        Content+= "\n- Instance ID: "+modelInstanceId+"\n- SpecificObjective:"+descriptionInstance+"\n- Date Creation Instance: "+date_create_instance+"\n-Date last modify: "+date_last_modify_instance;
		return Content;
    }
	/*
	...
	public void importDataInstance(FRAMModelInstance mi_old)
	{
	...
	public FRAMModelInstance cloneModelInstance(int modelInstanceId, int userInstanceId, Model model)
	{
		FRAMModelInstance mi = new FRAMModelInstance(modelInstanceId, specific_objective.concat("_cloned"), userInstanceId, model);
